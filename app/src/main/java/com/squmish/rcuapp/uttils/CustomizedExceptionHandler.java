...
public class CustomizedExceptionHandler implements UncaughtExceptionHandler {
    ...
    private void writeToFile(String currentStacktrace) {
        try {
            ...
            File dir1 = new File (root.getAbsolutePath() + "/download");
            //Gets the Android external storage directory & Create new folder Crash_Reports
            File dir = new File(dir1,
                    "Crash_Reports");
            FileWriter fileWriter = getFileWriter(currentStacktrace, dir);
            fileWriter.close();
        } catch (Exception e) {
    ...
    private static @NonNull FileWriter getFileWriter(String currentStacktrace, File dir) throws IOException {
        if (!dir.exists()) {
            dir.mkdirs();
        }
